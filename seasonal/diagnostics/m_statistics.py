...
class MStatisticsComputer:
    ...
    def _compute_m9(self, seasonal: pd.Series) -> float:
        ...
        seasonal_df = pd.DataFrame(
            {"value": seasonal, "year": seasonal.index.year, "month": seasonal.index.month}
        )
...
def validate_quality_thresholds(
    m_stats: Dict[str, float], thresholds: Optional[QualityThresholds] = None
) -> Dict[str, Any]:
    """
...
def store_diagnostics_to_db(
    diagnostics: Dict[str, Any], engine: Optional["sa.Engine"] = None
) -> Optional[int]:
    """
