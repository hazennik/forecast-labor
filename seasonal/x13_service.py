...
class X13Service:
    ...
    def __init__(self, x13_path: str = "x13as", work_dir: Optional[Path] = None):
        """
    ...
    def _verify_x13(self) -> bool:
        ...
        try:
            subprocess.run([self.x13_path, "-v"], capture_output=True, timeout=5)
            return True
    ...
    def run_seasonal_adjustment(
        ...
        try:
            ...
            if regressors is not None and len(regressors) > 0:
                logger.info(
                    f"Writing regressor matrix file with {len(regressors.columns)} regressors..."
                )
    ...
    def _parse_x13_series_file(self, file_path: Path) -> pd.Series:
        ...
        try:
            ...
            df = pd.read_csv(
                file_path, delim_whitespace=True, names=["date", "value"], skiprows=2  # Skip header
            )
            ...
            # Create series
            series = pd.Series(df["value"].values, index=df["date"], name=file_path.stem)
