...
from seasonal.diagnostics.m_statistics import (
    MStatisticsComputer,
    validate_quality_thresholds as validate_m_quality,
)
...
class SeasonalAdjustmentPipeline:
    ...
    def run(
        ...
        regressors = self._build_regressors(series_data.index[0].date(), extended_end_date, config)
        ...
        spec_content = self._generate_spec(series_name, series_data, regressors, config)

        # Step 4: Run X-13
        results = self._run_x13(series_name, series_data, spec_content, regressors)
    ...
    def _build_regressors(
        self, start_date: date, end_date: date, config: Dict[str, Any]
    ) -> pd.DataFrame:
        """
        ...
        if config.get("use_holiday_regressors", True):
            try:
                holiday_regressors = self.holiday_builder.build(start_date, end_date)
        ...
        if config.get("use_strike_regressors", True):
            try:
                strike_regressors = self.strike_builder.build(
                    start_date, end_date, min_workers=config.get("strike_min_workers", 10000)
                )
        ...
        if config.get("use_weather_regressors", True):
            try:
                weather_regressors = self.weather_builder.build(start_date, end_date)
    ...
    def _run_x13(
        self, series_name: str, series_data: pd.Series, spec_content: str, regressors: pd.DataFrame
    ) -> Dict[str, Any]:
        """
        Run X-13 adjustment
        ...
        try:
            ...
            try:
                m_stats = self._compute_m_statistics(series_name, series_data, output_results)
            ...
            try:
                q_stats = self._compute_q_statistics(series_name, output_results)
            ...
            try:
                ...
                m_stats_flat = {
                    k: v
                    for k, v in output_results.get("m_statistics", {}).items()
                    if not isinstance(v, dict)
                }
                q_stats_flat = {
                    k: v
                    for k, v in output_results.get("q_statistics", {}).items()
                    if not isinstance(v, dict)
                }
                ...
                output_results["quality_monitoring"] = {
                    ...
                    "quality_grade": self.quality_monitor.assess_quality_grade(
                        combined_diagnostics
                    ),
                }
    ...
    def _compute_m_statistics(
        self, series_name: str, original_series: pd.Series, x13_results: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        return {**m_stats, "m_quality_assessment": quality_assessment}

    def _compute_q_statistics(
        self, series_name: str, x13_results: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        Compute Q-statistics (Ljung-Box test) from irregular component
        ...
        return {**q_stats, "q_quality_assessment": quality_assessment}
    ...
    def run_batch(
        self, series_dict: Dict[str, pd.Series], configs: Optional[Dict[str, Dict[str, Any]]] = None
        ...
        for series_name, series_data in series_dict.items():
            try:
                ...
                result = self.run(series_name, series_data, series_data.index[0].date(), config)
...
if __name__ == "__main__":
    ...
    results = pipeline.run(
        series_name="test_series", series_data=series, start_date=dates[0].date()
    )
