...
class WeatherRegressors(RegressorBuilder):
    ...
    def build(
        ...
        regressors["weather_impact"] = self._build_impact_regressor(
            dates, weather_df
        )
        ...
        for event_type in event_types_in_data:
            ...
            if len(type_events) > 0:
                ...
                regressors[col_name] = self._build_impact_regressor(
                    dates, type_events
                )
    ...
    def _load_weather_data(
        ...
        try:
            ...
            weather_df = self.storage_client.read_parquet(
                "vintages/weather/latest/weather.parquet"
            )
    ...
    def build_winter_storm_regressor(
        self,
        start_date: date,
        end_date: date
    ) -> pd.Series:
