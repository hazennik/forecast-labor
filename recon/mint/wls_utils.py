...
def compute_shrinkage_covariance(
    ...
    if len(data) < 2:
        raise ValueError(
            f"At least 2 observations required, got {len(data)}"
        )
    ...
    if matrix_type == 'diagonal':
        ...
        try:
            ...
            if np.any(eigenvalues <= tolerance):
                return False, f"Matrix is not positive definite (min eigenvalue: {eigenvalues.min():.2e})"
        ...
        return True, f"Valid full covariance weight matrix (condition number: {condition_number:.2e})"
...
def compute_wls_weights(
    ...
    if method == 'ols':
        ...
        if variances is not None:
            ...
            raise ValueError(
                "Either 'variances' or 'data' required for 'diagonal' method"
            )
