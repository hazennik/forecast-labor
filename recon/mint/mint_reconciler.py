...
class MinTReconciler:
    ...
    def __init__(
        ...
        if hierarchy_type != "single_level":
            raise ValueError(f"Only 'single_level' hierarchy supported, got {hierarchy_type}")
        ...
        logger.info("Initialized MinTReconciler", method=method, hierarchy_type=hierarchy_type)

    def fit(
        self, forecasts: pd.DataFrame, forecast_errors: Optional[pd.DataFrame] = None
    ) -> "MinTReconciler":
        """
        ...
        if len(forecasts.columns) < 2:
            raise ValueError("forecasts must have at least 2 columns (1 total + 1 bottom)")
        ...
        self.summing_matrix_ = build_summing_matrix(
            n_bottom=self.n_bottom_, hierarchy_type=self.hierarchy_type
        )
    ...
    def reconcile(self, forecasts: pd.DataFrame) -> pd.DataFrame:
        """
        ...
        if not self.is_fitted_:
            raise ValueError("Reconciler must be fitted before reconciliation. Call fit() first.")

        # Validate structure matches fit
        if list(forecasts.columns) != self.column_order_:
            raise ValueError(
                f"Column mismatch. Expected {self.column_order_}, " f"got {list(forecasts.columns)}"
            )

        logger.debug("Reconciling forecasts", n_observations=len(forecasts), method=self.method)
        ...
        try:
            ...
            np.vstack([np.zeros((1, self.n_bottom_)), np.eye(self.n_bottom_)])
        ...
        reconciled_df = pd.DataFrame(reconciled, index=forecasts.index, columns=forecasts.columns)

        # Log reconciliation statistics
        coherence_errors_before = compute_coherence_errors(
            forecasts, forecasts.columns[0], list(forecasts.columns[1:])
        )

        coherence_errors_after = compute_coherence_errors(
            reconciled_df, reconciled_df.columns[0], list(reconciled_df.columns[1:])
        )
    ...
    def validate_coherence(self, forecasts: pd.DataFrame, tolerance: float = 1e-6) -> bool:
        """
    ...
    def _ensure_positive_definite(
        self, matrix: np.ndarray, min_eigenvalue: float = 1e-6
    ) -> np.ndarray:
        """
