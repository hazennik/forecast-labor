...
class TestQualityMonitorBasics:
    ...
    def test_record_single_diagnostic(self):
        ...
        diagnostics = {"m1": 0.25, "m2": 0.28, "m7": 0.45, "m8": 0.50, "q_statistic": 0.42}

        monitor.record_diagnostics(
            series_name="TEST_SERIES", diagnostics=diagnostics, timestamp=datetime(2024, 1, 1)
        )
...
class TestDegradationTrendDetection:
    ...
    def test_detect_no_degradation_stable_stats(self):
        ...
        for i in range(5):
            monitor.record_diagnostics(
                series_name="TEST_SERIES",
                diagnostics={"m7": 0.45, "m8": 0.50, "q_statistic": 0.42},  # Stable  # Stable
                timestamp=datetime(2024, 1, 1) + timedelta(days=i),
            )
    ...
    def test_detect_threshold_breach(self):
        ...
        monitor = QualityMonitor(thresholds={"m7": 1.0, "m8": 1.0, "q_statistic": 1.0})

        # Record stat exceeding threshold
        monitor.record_diagnostics(
            series_name="TEST_SERIES",
            diagnostics={"m7": 1.2, "m8": 0.5, "q_statistic": 0.4},  # Exceeds threshold of 1.0
...
class TestQualityScoring:
    ...
    def test_quality_score_decreases_with_degradation(self):
        ...
        poor_diagnostics = {"m7": 0.85, "m8": 0.90, "q_statistic": 0.95}
...
class TestMultipleSeriesMonitoring:
    ...
    def test_monitor_multiple_series(self):
        ...
        # Record for series 1
        monitor.record_diagnostics(
            series_name="SERIES_1", diagnostics={"m7": 0.40}, timestamp=datetime(2024, 1, 1)
        )

        # Record for series 2
        monitor.record_diagnostics(
            series_name="SERIES_2", diagnostics={"m7": 0.50}, timestamp=datetime(2024, 1, 1)
        )
