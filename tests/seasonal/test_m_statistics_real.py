...
class TestMStatisticsComputer:
    ...
    def test_q_statistic_is_average_of_m_stats(self, simple_decomposition):
        ...
        assert abs(result['q_statistic'] - expected_q) < 1e-6, \
    ...
    def test_m1_measures_irregular_contribution(self, simple_decomposition):
        ...
        assert m1_modified > m1_baseline, \
            "M1 should increase when irregular component is larger"
    ...
    def test_m7_detects_weak_seasonality(self):
        ...
        assert m7_weak > m7_strong, \
            "M7 should be higher when seasonality is weaker"
    ...
    def test_m9_detects_unstable_seasonality(self):
        ...
        assert m9_changing > m9_stable, \
            "M9 should be higher when seasonal factors are unstable"
...
class TestQualityThresholdValidation:
    ...
    def test_threshold_enforcement_m7(self):
        ...
        assert 'm7' in str(validation['warnings']) or 'm7' in str(validation['failures']), \
    ...
    def test_threshold_enforcement_m8(self):
        ...
        assert 'm8' in str(validation['warnings']) or 'm8' in str(validation['failures']), \
...
class TestDeterminism:
    ...
    def test_same_input_produces_same_output(self, simple_decomposition):
        ...
        for i in range(1, 12):
            m_key = f'm{i}'
            assert abs(result1[m_key] - result2[m_key]) < 1e-10, \
                f"{m_key} should be deterministic"
    ...
    def test_computation_is_reproducible_across_runs(self, high_quality_decomposition):
        ...
        for i in range(1, 12):
            m_key = f'm{i}'
            assert result1[m_key] == result2[m_key], \
