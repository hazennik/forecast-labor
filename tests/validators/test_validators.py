...
class TestSchemaValidator:
    ...
    def sample_schema(self):
        ...
        return {
            ...
            "column_types": {"date": "datetime64[ns]", "value": "int64", "category": "object"},
            "unique_columns": ["date"],
...
class TestFreshnessValidator:
    ...
    def test_freshness_validator_initialization(self):
        ...
        validator = FreshnessValidator(
            source_name="test_source", date_column="date", max_age_days=7
        )
    ...
    def test_validates_fresh_data(self):
        ...
        fresh_df = pd.DataFrame({"date": [datetime.now() - timedelta(days=1)], "value": [100]})
    ...
    def test_validates_stale_data(self):
        ...
        stale_df = pd.DataFrame({"date": [datetime.now() - timedelta(days=30)], "value": [100]})
...
class TestQualityValidator:
    ...
    def test_validates_null_values(self):
        ...
        df_with_nulls = pd.DataFrame({"value": [1, 2, None, None, 5, 6, 7, 8, 9, 10]})
    ...
    def test_validates_duplicates(self):
        ...
        validator = QualityValidator(
            source_name="test_source", unique_keys=["id"]  # Will check for duplicate keys
        )

        # DataFrame with duplicate IDs
        df_with_dupes = pd.DataFrame(
            {"id": [1, 1, 2, 3, 4], "value": [100, 200, 300, 400, 500]}  # ID 1 is duplicated
        )
    ...
    def test_generate_html_report(self, sample_results):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            output_path = generator.generate_html_report(
                results=sample_results, title="Test Validation Report", output_file=output_file
            )
    ...
    def test_generate_csv_report(self, sample_results):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            output_path = generator.generate_csv_summary(
                results=sample_results, output_file=output_file
            )
...
class TestValidatorIntegration:
    ...
    def test_full_validation_workflow(self):
        ...
        quality_validator = QualityValidator(
            "test_source", critical_columns=["value"], numeric_ranges={"value": (0, 200)}
        )
    ...
    def test_validators_with_invalid_data(self):
        ...
        quality_validator = QualityValidator(
            "test_source", critical_columns=["value"]  # Nulls will be flagged
        )
