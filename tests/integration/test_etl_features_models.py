...
def generate_features(data: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    features_df = data[["nfp_employment", "initial_claims", "treasury_withholdings", "ces_employment"]].copy()
...
def register_features_to_registry(
    ...
    for feature_name in feature_names:
        feature_dict = {
            ...
            "frequency": "monthly",
            "transform": "lag" if "lag" in feature_name else ("rolling_mean" if "ma" in feature_name else "raw"),
...
class TestETLFeaturesModelsIntegration:
    ...
    def test_complete_pipeline_with_midas(
        ...
        feature_ids = register_features_to_registry(feature_names, feature_registry_memory, integration_vintage_date)
        ...
        np.testing.assert_array_almost_equal(predictions, predictions_2, decimal=5,
    ...
    def test_complete_pipeline_with_xgboost(
        ...
        # Step 8: Verify reproducibility
        model_2 = XGBoostQuantile(quantiles=[0.1, 0.5, 0.9], n_estimators=20, max_depth=3, random_state=42)
        model_2.fit(X_train_df, y_train_series, vintage_date=str(integration_vintage_date))
        predictions_dict_2 = model_2.predict(X_test_df)
        
        # Compare dicts
        for q in [0.1, 0.5, 0.9]:
            np.testing.assert_array_equal(predictions_dict[q], predictions_dict_2[q],
    ...
    def test_complete_pipeline_with_lightgbm(
        ...
        # Step 8: Verify reproducibility
        model_2 = LightGBMQuantile(quantiles=[0.1, 0.5, 0.9], n_estimators=20, max_depth=3, random_state=42)
        model_2.fit(X_train_df, y_train_series, vintage_date=str(integration_vintage_date))
        predictions_dict_2 = model_2.predict(X_test_df)
        
        # Compare dicts
        for q in [0.1, 0.5, 0.9]:
            np.testing.assert_array_almost_equal(predictions_dict[q], predictions_dict_2[q], decimal=3,
