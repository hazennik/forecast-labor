...
class TestValidationStep:
    ...
    def test_validation_step_creation(self):
        ...
        step = ValidationStep(
            step_id="test_step", step_name="Test Step", status=ValidationStatus.PASSED
        )
...
class TestValidationReport:
    ...
    def test_validation_report_creation(self):
        ...
        report = ValidationReport(
            validation_id="test_validation_123", started_at="2025-01-01T00:00:00"
        )
...
class TestAPIKeysCheck:
    ...
    @patch.dict(
        os.environ,
        {"BLS_API_KEY": "test_key", "NOAA_API_TOKEN": "test_token", "ALLOW_FALLBACK_DATA": "false"},
    )
    ...
    def test_fallback_data_enabled_warning(self):
        ...
        assert step.status == ValidationStatus.PASSED
        assert any("ALLOW_FALLBACK_DATA" in issue for issue in validator.report.issues_discovered)
        assert any("ALLOW_FALLBACK_DATA=false" in rec for rec in validator.report.recommendations)
...
class TestInfrastructureCheck:
    ...
    def test_infrastructure_check_passed(self, mock_run):
        ...
        mock_run.return_value = Mock(returncode=0, stdout="All services healthy", stderr="")
    ...
    def test_infrastructure_check_failed(self, mock_run):
        ...
        mock_run.return_value = Mock(returncode=1, stdout="", stderr="Postgres not healthy")
        ...
        assert any("Docker services" in issue for issue in validator.report.issues_discovered)
...
class TestRealETL:
    ...
    def test_real_etl_passed(self, mock_run):
        ...
        mock_run.return_value = Mock(
            returncode=0, stdout="✅ All ETL pipelines completed successfully", stderr=""
        )
    ...
    def test_real_etl_failed(self, mock_run):
        ...
        mock_run.return_value = Mock(returncode=1, stdout="", stderr="API authentication failed")
        ...
        assert step.status == ValidationStatus.FAILED
        assert "failed" in step.error_message.lower()
        assert any("Real ETL failed" in issue for issue in validator.report.issues_discovered)
...
class TestDataQualityValidation:
    ...
    def test_data_quality_validation_passed(self, mock_run):
        ...
        mock_run.return_value = Mock(returncode=0, stdout="✅ All validations passed", stderr="")
    ...
    def test_data_quality_validation_failed(self, mock_run):
        ...
        mock_run.return_value = Mock(
            returncode=1, stdout="", stderr="Schema validation failed: Missing required columns"
        )
...
class TestSeasonalAdjustment:
    ...
    def test_seasonal_adjustment_passed(self, mock_run):
        ...
        mock_run.return_value = Mock(
            returncode=0, stdout="✅ Seasonal adjustment completed", stderr=""
        )
...
class TestFeatureBuilding:
    ...
    def test_feature_building_passed(self, mock_run):
        ...
        mock_run.return_value = Mock(
            returncode=0, stdout="✅ Features built successfully", stderr=""
        )
...
class TestReportGeneration:
    ...
    @patch.dict(os.environ, {"BLS_API_KEY": "test_key", "NOAA_API_TOKEN": "test_token"})
    def test_generate_json_report(self):
        ...
        step1 = ValidationStep(
            step_id="1_api_keys", step_name="API Keys Check", status=ValidationStatus.PASSED
        )
