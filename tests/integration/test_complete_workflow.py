...
from models_src.utils.signing import create_signed_bundle, extract_signed_bundle, TamperDetectedError
...
class TestComponentIntegration:
    ...
    def test_calibrated_to_revision(self, vintage_data, train_val_test_indices):
        ...
        preliminary_forecasts = y_train.mean() + np.random.randn(len(y_train)) * 300
        ...
        revision_model.fit(X_train, pd.Series(true_revisions, index=X_train.index), vintage_date='2024-01-15')
    ...
    def test_revised_to_mint_reconciliation(self, vintage_data, train_val_test_indices):
        ...
        base_forecasts = pd.DataFrame({
            'national': national.iloc[test_idx[0]:test_idx[1]].values + np.random.randn(len(test_dates)) * 100,
            'CA': states['CA'].iloc[test_idx[0]:test_idx[1]].values + np.random.randn(len(test_dates)) * 50,
            'TX': states['TX'].iloc[test_idx[0]:test_idx[1]].values + np.random.randn(len(test_dates)) * 50,
            'NY': states['NY'].iloc[test_idx[0]:test_idx[1]].values + np.random.randn(len(test_dates)) * 50,
        }, index=test_dates)
        ...
        reconciled_incoherence = np.abs(reconciled_sum_states - reconciled_forecasts['national']).max()
...
class TestCompletePipeline:
    ...
    def test_complete_pipeline_simple(self, vintage_data, train_val_test_indices):
        ...
        states_target = vintage_data['states']
        ...
        ca_pred = revised_predictions * 0.40 + np.random.RandomState(42).randn(len(revised_predictions)) * 100
        tx_pred = revised_predictions * 0.35 + np.random.RandomState(43).randn(len(revised_predictions)) * 100
        ny_pred = revised_predictions * 0.25 + np.random.RandomState(44).randn(len(revised_predictions)) * 100
        ...
        assert relative_coherence_error < 1e-6, f"Relative coherence error {relative_coherence_error:.2e} too large"
...
class TestCoherenceValidation:
    ...
    def test_mint_ols_vs_shrink_differ(self, vintage_data, train_val_test_indices):
        ...
        diff = np.abs(reconciled_ols['national'].values - reconciled_shrink['national'].values).max()
    ...
    def test_mint_preserves_information(self, vintage_data, train_val_test_indices):
        ...
        for col in base_forecasts.columns:
            relative_change = np.abs(reconciled[col] - base_forecasts[col]) / np.abs(base_forecasts[col])
...
class TestMLflowIntegration:
    ...
    def test_mlflow_model_registration(self, temp_mlflow_dir, temp_model_dir, vintage_data, train_val_test_indices):
