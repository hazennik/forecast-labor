...
class TestUIClaimsETL:
    ...
    def temp_paths(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            tmppath = Path(tmpdir)
            yield {"raw_data": tmppath / "raw" / "claims", "vintage": tmppath / "vintages"}
...
"""
    ...
    @patch("etl.common.downloader.Downloader.download")
    def test_full_pipeline_creates_files(
        self, mock_download, claims_etl, mock_claims_csv, temp_paths
    ):
    ...
    @patch("etl.common.downloader.Downloader.download")
    def test_full_pipeline_vintage_data_loadable(
        self, mock_download, claims_etl, mock_claims_csv, temp_paths
    ):
