...
class TestRevisionForecaster:
    ...
    def sample_data(self):
        ...
        revisions = (
            -0.1 * (preliminary_values - 150)
            + -0.05 * (claims_4wk - 220)  # Mean reversion
            + 0.3 * prev_revision_avg  # Claims signal
            + np.random.randn(n_samples) * 5  # Persistence  # Noise
        )
    ...
    def fitted_model(self, sample_data):
        ...
        X, y = sample_data
        model = RevisionForecaster(alpha=1.0, fit_intercept=True, random_state=42)
    ...
    def test_init_custom_parameters(self):
        ...
        model = RevisionForecaster(alpha=0.5, fit_intercept=False, random_state=123)
    ...
    def test_save_load_roundtrip(self, fitted_model, sample_data):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert np.allclose(loaded_model.model_.coef_, fitted_model.model_.coef_)
            assert np.isclose(loaded_model.model_.intercept_, fitted_model.model_.intercept_)
...
class TestRevisionForecasterEdgeCases:
    ...
    def test_single_feature(self):
        ...
        np.random.seed(42)
        X = pd.DataFrame(np.random.randn(50, 1), columns=["preliminary_value"])
    ...
    def test_no_intercept(self):
        ...
        model = RevisionForecaster(alpha=1.0, fit_intercept=False, random_state=42)
        model.fit(X, y, vintage_date="2024-11-15")
...
class TestRevisionForecasterRealism:
    ...
    def test_persistence_pattern(self):
        ...
        importance = model.get_feature_importance()
        prev_rev_coef = importance[importance["feature"] == "prev_revision_avg"][
            "coefficient"
        ].values[0]
