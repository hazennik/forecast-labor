...
class MockForecaster(BaseForecaster):
    ...
    def predict(self, X):
        ...
        predictions = (
            X.iloc[:, 0].values + np.random.RandomState(self.random_state).randn(len(X)) * 0.1
        )
        return predictions
...
class TestTimeSeriesSplits:
    ...
    def test_no_data_leakage(self, sample_time_series_data, sample_training_config):
        ...
        assert train_max_date <= pd.Timestamp(
            sample_training_config.train_end_date
        ), "Training data includes dates after train_end_date (data leakage)"

        assert val_min_date >= pd.Timestamp(
            sample_training_config.val_start_date
    ...
    def test_chronological_order(self, sample_time_series_data, sample_training_config):
        ...
        assert train_max_date < val_min_date, "Training and validation data overlap"

        # Validation must be entirely before test
        assert val_max_date < test_min_date, "Validation and test data overlap"
    ...
    def test_vintage_date_awareness(self, sample_time_series_data, sample_training_config):
        ...
        assert (
            splits.X_train.index.max() <= vintage_date
        ), "Training data includes dates after vintage_date"
        assert (
            splits.X_val.index.max() <= vintage_date
        ), "Validation data includes dates after vintage_date"
        assert (
            splits.X_test.index.max() <= vintage_date
...
class TestModelTraining:
    ...
    def test_train_model_vintage_tracking(
        self, sample_time_series_data, sample_training_config, mock_model
    ):
    ...
    def test_train_model_feature_names(
        self, sample_time_series_data, sample_training_config, mock_model
    ):
...
class TestModelEvaluation:
    ...
    def test_evaluate_multiple_splits(
        self, sample_time_series_data, sample_training_config, mock_model
    ):
...
class TestTrainingPipeline:
    ...
    def test_pipeline_mlflow_logging(
        self, mock_mlflow_class, sample_time_series_data, sample_training_config, mock_model
    ):
    ...
    def test_pipeline_feature_registry_integration(
        self,
        mock_get_registry,
    ...
    def test_pipeline_no_model_saving(
        self, sample_time_series_data, sample_training_config, mock_model
    ):
    ...
    def test_pipeline_data_leakage_prevention(
        self, sample_time_series_data, sample_training_config, mock_model
    ):
        """
    ...
    def test_pipeline_reproducibility(self, sample_time_series_data, sample_training_config):
        ...
        for split in ["train", "validation", "test"]:
            for metric in ["rmse", "mae"]:
                assert (
                    abs(result1["metrics"][split][metric] - result2["metrics"][split][metric])
                    < 1e-6
...
class TestPrefectIntegration:
    ...
    def test_prefect_flow_structure(
        self, mock_flow, mock_task, sample_time_series_data, sample_training_config, mock_model
    ):
