...
class MockFastModel(BaseForecaster):
    ...
    def get_params(self, deep: bool = True) -> dict:
        ...
        return {"sleep_time": self.sleep_time, "random_state": self.random_state}
...
class TestTrainingPipelinePerformance:
    ...
    def test_training_time_under_baseline(self, large_dataset):
        ...
        assert elapsed < baseline_time, f"Training took {elapsed:.3f}s (baseline: {baseline_time}s)"
    ...
    def test_prediction_latency_acceptable(self, large_dataset):
        ...
        assert avg_latency < 0.1, f"Average latency {avg_latency*1000:.1f}ms (limit: 100ms)"
    ...
    def test_throughput_acceptable(self, large_dataset):
        ...
        assert throughput >= 10, f"Throughput: {throughput:.1f} pred/s (minimum: 10 pred/s)"
...
class TestPerformanceBaselines:
    ...
    def test_baseline_comparison(self, tmp_path):
        ...
        for metric in baseline:
            ...
            print(
                f"{metric}: {current_val:.3f} (baseline: {baseline_val:.3f}, "
                f"max: {max_allowed:.3f}) - {'✅ PASS' if within_tolerance else '❌ FAIL'}"
            )

            # For this test, we allow some variation
            assert (
                within_tolerance or current_val < baseline_val
