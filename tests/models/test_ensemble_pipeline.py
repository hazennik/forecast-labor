...
class MockForecaster(BaseForecaster):
    ...
    def predict(self, X):
        ...
        predictions = X.iloc[:, 0].values + self.bias + np.random.randn(len(X)) * self.variance
        return predictions
...
class TestUtilityFunctions:
    ...
    def test_validate_predictions_dict_valid(self, sample_predictions):
        ...
        # Should not raise
        validate_predictions_dict(sample_predictions, expected_keys=["dfm", "midas", "xgboost"])
    ...
    def test_validate_predictions_dict_missing_keys(self, sample_predictions):
        ...
        with pytest.raises(ValueError, match="Missing predictions"):
            validate_predictions_dict(
                sample_predictions, expected_keys=["dfm", "midas", "xgboost", "missing_model"]
            )
    ...
    def test_validate_predictions_dict_mismatched_lengths(self):
        ...
        with pytest.raises(ValueError, match="same length"):
            validate_predictions_dict(predictions, expected_keys=["model1", "model2"])
    ...
    def test_simple_average(self, sample_predictions):
        ...
        expected = (
            sample_predictions["dfm"] + sample_predictions["midas"] + sample_predictions["xgboost"]
        ) / 3
    ...
    def test_optimize_weights_improves_performance(self, sample_predictions, sample_true_values):
        """
...
class TestEnsembleForecasterPrediction:
    ...
    def test_predict_simple_average(self, fitted_mock_models, sample_data):
        ...
        individual_preds = {
            name: model.predict(X_test) for name, model in fitted_mock_models.items()
        }
...
class TestIntegration:
    ...
    def test_ensemble_with_missing_model(self, fitted_mock_models, sample_data):
        ...
        sample_data[["feat1", "feat2", "feat3"]].iloc[150:160]
