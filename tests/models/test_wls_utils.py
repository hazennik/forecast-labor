...
class TestVarianceWeights:
    ...
    def test_compute_variance_weights_with_zeros(self):
        ...
        data = pd.DataFrame({"constant": [5, 5, 5, 5, 5], "variable": [1, 2, 3, 4, 5]})
...
class TestSampleCovariance:
    ...
    def test_compute_sample_covariance_basic(self):
        ...
        data = pd.DataFrame({"series1": [1, 2, 3, 4, 5], "series2": [2, 4, 6, 8, 10]})
    ...
    def test_compute_sample_covariance_positive_definite(self):
        ...
        np.random.seed(42)
        data = pd.DataFrame(np.random.randn(100, 5), columns=[f"series{i}" for i in range(5)])
    ...
    def test_compute_sample_covariance_insufficient_data(self):
        ...
        data = pd.DataFrame(
            np.random.randn(3, 5), columns=[f"series{i}" for i in range(5)]  # 3 obs, 5 variables
        )
...
class TestShrinkageCovariance:
    ...
    def test_compute_shrinkage_covariance_basic(self):
        ...
        np.random.seed(42)
        data = pd.DataFrame(np.random.randn(50, 4), columns=[f"series{i}" for i in range(4)])
    ...
    def test_compute_shrinkage_covariance_small_sample(self):
        ...
        data = pd.DataFrame(np.random.randn(10, 8), columns=[f"series{i}" for i in range(8)])
...
class TestPositiveDefinite:
    ...
    def test_ensure_positive_definite_negative_eigenvalues(self):
        ...
        matrix = np.array([[1, 0, 0], [0, -1, 0], [0, 0, 1]])  # Negative eigenvalue
    ...
    def test_ensure_positive_definite_symmetry(self):
        ...
        matrix = np.array([[2, 1, 0], [1.001, 2, 1], [0, 1, 2]])  # Slight asymmetry
...
class TestWeightMatrixValidation:
    ...
    def test_validate_weight_matrix_not_positive_definite(self):
        ...
        weights = np.array(
            [[1, 0, 0], [0, -1, 0], [0, 0, 1]]  # Negative diagonal will be caught first
        )
...
class TestWLSWeights:
    ...
    def test_compute_wls_weights_ols(self):
        ...
        weights = compute_wls_weights(n_series=n_series, method="ols")
    ...
    def test_compute_wls_weights_diagonal(self):
        ...
        weights = compute_wls_weights(variances=variances, method="diagonal")
    ...
    def test_compute_wls_weights_from_data(self):
        ...
        data = pd.DataFrame({"series1": [1, 2, 3, 4, 5], "series2": [10, 20, 30, 40, 50]})

        weights = compute_wls_weights(data=data, method="sample")
...
class TestIntegration:
    ...
    def test_full_wls_workflow(self):
        ...
        data = pd.DataFrame(
            np.random.randn(100, 4), columns=["national", "state1", "state2", "state3"]
        )
    ...
    def test_wls_weights_consistency(self):
        ...
        np.random.seed(42)
        data = pd.DataFrame(np.random.randn(50, 3), columns=["total", "component1", "component2"])
