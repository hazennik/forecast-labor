...
class TestAlmonWeightProperties:
    ...
    def fitted_model(self):
        ...
        X = pd.DataFrame(
            {f"lag_{i}": np.random.normal(100, 10, n_obs) + i * 0.5 for i in range(20)}, index=dates
        )
    ...
    def test_almon_weights_non_negative(self, fitted_model):
        ...
        assert min_weight >= -1e-10, f"Almon weights have negative values: min = {min_weight:.2e}"
    ...
    def test_almon_weights_all_finite(self, fitted_model):
        ...
        assert np.all(np.isfinite(weights)), f"Almon weights contain non-finite values: {weights}"
...
class TestNLSOptimizationProperties:
    ...
    def sample_data(self):
        ...
        y = pd.Series(y_signal + np.random.normal(0, 10, n_obs), index=dates, name="nfp_change")
    ...
    def test_nls_loss_non_negative(self, sample_data):
        ...
        assert loss >= 0, f"Training loss is negative: {loss:.2e}"
    ...
    def test_nls_predictions_match_loss(self, sample_data):
        ...
        assert (
            np.abs(computed_loss - model.training_loss_) < 1e-6
...
class TestMIDASCoefficientsProperties:
    ...
    def test_coefficients_all_finite(self, fitted_model):
        ...
        assert np.all(np.isfinite(coefs)), f"Coefficients contain non-finite values: {coefs}"
    ...
    def test_intercept_finite(self, fitted_model):
        ...
        assert np.isfinite(intercept), f"Intercept is not finite: {intercept}"
    ...
    def test_coefficients_reasonable_magnitude(self, fitted_model):
        ...
        assert max_coef < 1e6, f"Coefficients have very large magnitude: max |β| = {max_coef:.2e}"
...
class TestMIDASIntegrationWithProperties:
    ...
    def test_weights_and_loss_consistency(self):
        ...
        X = pd.DataFrame(
            {f"lag_{i}": np.random.normal(100, 10, n_obs) for i in range(15)}, index=dates
        )
    ...
    def test_reproducibility_implies_stable_optimization(self):
        ...
        X = pd.DataFrame(
            {f"lag_{i}": np.random.normal(100, 10, n_obs) for i in range(10)}, index=dates
        )
        ...
        assert (
            np.abs(model1.training_loss_ - model2.training_loss_) < 1e-10
