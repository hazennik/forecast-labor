...
class TestFoldGeneration:
    ...
    def test_chronological_ordering(self, sample_time_series_data, sample_cv_config):
        ...
        for i in range(len(folds) - 1):
            ...
            assert next_train_min <= current_test_max, "Chronological order violation"
    ...
    def test_vintage_date_constraint(self, sample_time_series_data, sample_cv_config):
        ...
        for i, fold in enumerate(folds):
            assert (
                fold.X_train.index.max() <= vintage_date
            ), f"Fold {i}: Training data exceeds vintage date"
            assert (
                fold.X_test.index.max() <= vintage_date
    ...
    def test_forecast_horizon_consistency(self, sample_time_series_data, sample_cv_config):
        ...
        for fold in folds:
            ...
            assert (
                len(fold.X_test) <= sample_cv_config.forecast_horizon
...
class TestCrossValidationExecution:
    ...
    def test_model_trained_on_each_fold(
        self, sample_time_series_data, sample_cv_config, mock_model
    ):
...
class TestCrossValidationTimeouts:
    ...
    def sample_data(self):
        ...
        y = pd.Series(np.random.randn(n_samples) * 1000 + 150000, index=dates, name="target")
        return X, y
    ...
    def test_cv_config_default_timeout_none(self, sample_data):
        ...
        assert (
            getattr(config, "max_time_per_fold_seconds", None) is None
            or config.max_time_per_fold_seconds is None
        )
        assert (
            getattr(config, "total_max_time_seconds", None) is None
            or config.total_max_time_seconds is None
        )
...
class TestTimeoutDocumentation:
    ...
    def test_timeout_parameters_have_type_hints(self):
        ...
        if "max_time_per_fold_seconds" in hints:
            ...
            assert (
                origin is not None or hint_type == int
...
class TestTimeoutEnforcement:
    ...
    def test_per_fold_timeout_logs_warning(self, timeout_data, fast_config, caplog):
        ...
        assert (
            len(timeout_warnings) > 0 or len(results) == 0
