...
class TestMinTReconcilerFit:
    ...
    def test_fit_wls_method(self, sample_forecasts):
        ...
        # Should be diagonal
        assert np.allclose(reconciler.weight_matrix_, np.diag(np.diag(reconciler.weight_matrix_)))
    ...
    def test_fit_mint_sample_method(self, sample_forecasts):
        ...
        assert reconciler.weight_matrix_.shape == (4, 4)
        # Should be symmetric
        assert np.allclose(reconciler.weight_matrix_, reconciler.weight_matrix_.T)
...
class TestMinTReconcilerReconcile:
    ...
    def test_reconcile_coherence_error_reduction(self, fitted_reconciler, incoherent_forecasts):
        """Test that reconciliation reduces coherence error."""
        # Compute coherence errors before
        errors_before = incoherent_forecasts["national"] - incoherent_forecasts[
            ["CA", "TX", "NY"]
        ].sum(axis=1)
        ...
        errors_after = reconciled["national"] - reconciled[["CA", "TX", "NY"]].sum(axis=1)
    ...
    def test_reconcile_coherence_error_under_threshold(
        ...
        coherence_error = np.abs(
            reconciled["national"] - reconciled[["CA", "TX", "NY"]].sum(axis=1)
        ).max()
...
class TestMinTReconcilerMethods:
    ...
    def test_method_comparison_forecast_accuracy(self, base_forecasts):
        ...
        for method in ["ols", "wls", "mint_sample", "mint_shrink"]:
            ...
            rmse = np.sqrt(np.mean((reconciled["national"] - true_values) ** 2))
            results[method] = rmse

        # All methods should improve over base
        base_rmse = np.sqrt(np.mean((base_forecasts["national"] - true_values) ** 2))

        for method, rmse in results.items():
            assert rmse <= base_rmse * 1.1, f"Method {method} did not improve forecasts"
...
class TestMinTReconcilerEdgeCases:
    ...
    def test_reconcile_many_bottom_series(self):
        ...
        assert np.allclose(
            reconciled["national"].values, reconciled.iloc[:, 1:].sum(axis=1).values, atol=1e-5
        )
