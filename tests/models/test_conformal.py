...
class TestConformalPredictorInitialization:
    ...
    def test_init_custom_parameters(self):
        ...
        predictor = ConformalPredictor(
            confidence_levels=custom_levels, adaptive=True, adaptive_gamma=0.2
        )
    ...
    def test_coverage_multiple_levels(self):
        ...
        for conf_level in [0.8, 0.9, 0.95]:
            metrics = predictor.validate_coverage(
                y_test_true, y_test_pred, confidence_level=conf_level
            )
...
class TestIntervalWidth:
    ...
    def test_interval_width_consistent_for_fixed_predictor(self):
        ...
        predictor = ConformalPredictor(confidence_levels=[0.9], adaptive=False)
        predictor.fit(y_calib_true, y_calib_pred)
    ...
    def test_adaptive_intervals_vary_by_prediction(self):
        ...
        predictor = ConformalPredictor(confidence_levels=[0.9], adaptive=True, adaptive_gamma=0.1)
        predictor.fit(y_calib_true, y_calib_pred)
...
class TestSaveLoad:
    ...
    def test_save_load_roundtrip(self):
        ...
        predictor = ConformalPredictor(
            confidence_levels=[0.8, 0.9, 0.95], adaptive=True, adaptive_gamma=0.15
        )
        predictor.fit(y_calib_true, y_calib_pred)
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for key in predictor.quantiles_:
                assert np.isclose(loaded_predictor.quantiles_[key], predictor.quantiles_[key])

            # Check intervals match
            lower_loaded, upper_loaded = loaded_predictor.predict_interval(
                test_pred, confidence_level=0.9
            )
...
class TestGetParams:
    ...
    def test_get_params_unfitted(self):
        ...
        predictor = ConformalPredictor(confidence_levels=[0.85, 0.95], adaptive=True)
...
class TestIntegration:
    ...
    def test_full_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            lower_loaded, upper_loaded = loaded_predictor.predict_interval(
                y_test_pred, confidence_level=0.9
            )
