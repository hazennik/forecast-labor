...
class TestFeatureRegistration:
    ...
    def test_register_feature_to_database(self, database_backend, sample_feature_data, mock_db_connection):
        ...
        calls = [str(arg[0]) if len(arg) > 0 else '' for arg in [call[0] for call in mock_cursor.execute.call_args_list]]
        assert any("INSERT INTO features.feature_metadata" in call for call in calls)
        assert feature_id == 'test-uuid-123'
    
    def test_register_feature_with_lineage(self, database_backend, sample_feature_data, mock_db_connection):
    ...
    def test_register_feature_creates_initial_version(self, database_backend, sample_feature_data, mock_db_connection):
...
class TestLineageTracking:
    ...
    def test_get_feature_lineage(self, database_backend, mock_db_connection):
        ...
        mock_cursor.fetchall.return_value = [
            {'feature_id': 'uuid1', 'feature_name': 'feature1', 'transform_type': 'diff', 'depth': 0},
            {'feature_id': 'uuid2', 'feature_name': 'feature2', 'transform_type': 'seasonal_adj', 'depth': 1},
        ]
    ...
    def test_get_feature_descendants(self, database_backend, mock_db_connection):
        ...
        # Mock RealDictCursor results
        mock_cursor.fetchall.return_value = [
            {'feature_id': 'child-uuid-1', 'feature_name': 'child_feature1', 'transform_type': 'diff'},
            {'feature_id': 'child-uuid-2', 'feature_name': 'child_feature2', 'transform_type': 'log'},
        ]
...
class TestVersioning:
    ...
    def test_create_feature_version(self, database_backend, mock_db_connection):
        ...
        mock_cursor.fetchone.side_effect = [(2,), ('version-uuid-123',)]  # version number, then version_id
        
        version_id = database_backend.create_version(
            feature_id='feature-uuid',
            checksum='abc123',
            row_count=1000,
...
class TestFeatureRegistryDatabaseIntegration:
    ...
    def test_registry_switches_between_backends(self, mock_backend_class):
        ...
        id2 = registry_db.register({'name': 'db_feature'})
