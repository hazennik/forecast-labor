...
class TestBatchOperationsPerformance:
    ...
    def test_bulk_update_performance(self, perf_registry, large_feature_set):
        ...
        for feature_id in update_ids:
            perf_registry._db_backend.update_feature(
                feature_id, {"description": "Updated for performance test"}
            )
...
class TestMemoryModePerformance:
    ...
    def test_memory_mode_registration_speed(self):
        ...
        features = [{"name": f"memory_test_feature_{i}", "source": "ces"} for i in range(100)]
    ...
    def test_memory_mode_retrieval_speed(self):
        ...
        features = [{"name": f"memory_test_feature_{i}", "source": "laus"} for i in range(100)]
        feature_ids = registry.bulk_register(features)
        ...
        assert (
            avg_time < 0.001
        ), f"Average memory retrieval time {avg_time:.3f}s (expected < 0.001s)"
...
class TestPerformanceRegression:
    ...
    def test_search_performance_with_growing_dataset(self, perf_registry):
        ...
        for batch_size in [50, 50, 50, 50]:
            ...
            # Measure search time
            start_time = time.time()
            perf_registry.search(source="scaling_test")
            search_time = time.time() - start_time
        ...
        try:
            ...
            assert (
                search_times[-1] < search_times[0] * 2
