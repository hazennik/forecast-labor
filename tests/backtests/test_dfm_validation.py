...
def load_vintage_data(
    vintage_manager: VintageManager, vintage_date: date
...
def real_vintage_dates(vintage_manager: VintageManager) -> List[date]:
    ...
    if not all_vintages:
        pytest.skip(
            "No BLS CES vintages available. Run ETL and create_historical_vintages.py first."
        )
...
def vintage_datasets(
    vintage_manager: VintageManager, real_vintage_dates: List[date]
) -> Dict[date, Dict]:
    """
...
class TestDFMNumericalStability:
    ...
    def test_dfm_stability_on_real_data(self, vintage_datasets: Dict[date, Dict]):
        """
        ...
        finite_count = sum(
            ...
            if not r.get("has_nan", True) and not r.get("has_inf", True)
        )
...
class TestDFMAccuracy:
    ...
    def test_dfm_accuracy_on_real_data(self, vintage_datasets: Dict[date, Dict]):
        """
...
class TestModelComparison:
    ...
    def test_model_comparison_on_real_data(self, vintage_datasets: Dict[date, Dict]):
        """
        ...
        for vintage_date, data in vintage_datasets.items():
            ...
            try:
                midas = MIDASRegression(
                    n_lags=min(10, X_train.shape[1]), almon_degree=2, random_state=42
                )
            ...
            try:
                xgb = XGBoostQuantile(
                    quantiles=[0.5], n_estimators=50, max_depth=3, random_state=42
                )
...
class TestDFMCalibrationIntegration:
    ...
    def test_bridge_to_calibration_pipeline(
        ...
        pytest.dfm_calibration_summary = {
            ...
            "recommendation": (
                "calibration_pass"
                if coverage_within_target and ece_meets_target
    ...
    def test_calibration_with_ragged_edge(self, vintage_datasets: Dict[date, Dict]):
        ...
        vintage_date, data = next(iter(vintage_datasets.items()))
        target = data["target"]
        raw_sources = {name: frame.iloc[:-1].copy() for name, frame in data["raw_sources"].items()}
        bridge = MIDASBridge(source_configs=data["source_configs"])
        features = bridge.build_features(
            raw_sources, pd.DatetimeIndex(target.index), str(vintage_date)
        )
...
class TestDFMValidationSummary:
    ...
    def test_generate_validation_report(
        self, vintage_datasets: Dict[date, Dict], real_vintage_dates: List[date]
    ):
        """
        ...
        report_lines = [
            ...
            "Data source: BLS CES API (real employment data)",
            "",
        ]
        ...
        dfm_meets_accuracy = (
            avg_dfm_smape < SMAPE_THRESHOLD if not np.isnan(avg_dfm_smape) else False
        )
        ...
        report_lines.extend(
            [
                ...
                f"  - Average DFM sMAPE: {avg_dfm_smape:.2f}%"
                if not np.isnan(avg_dfm_smape)
                else "  - Average DFM sMAPE: N/A (unstable)",
        ...
        if all_stable and dfm_meets_accuracy:
            ...
            if not all_stable:
                report_lines.append(
                    f"  - DFM shows numerical instability ({stable_count}/{total_tests} stable)"
                )
            if not dfm_meets_accuracy:
                report_lines.append(
                    f"  - DFM accuracy below threshold (sMAPE: {avg_dfm_smape:.2f}% > {SMAPE_THRESHOLD}%)"
                )
