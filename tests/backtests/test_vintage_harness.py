...
class TestVintageReconstruction:
    ...
    def test_reconstruct_single_source(self, vintage_harness, sample_vintages):
        """Test reconstructing state for a single data source."""
        as_of_date = date(2024, 2, 15)
        source = "ces"

        state = vintage_harness.reconstruct_state(as_of_date=as_of_date, sources=[source])
    ...
    def test_reconstruct_multiple_sources(self, vintage_harness, sample_vintages):
        """Test reconstructing state for multiple data sources."""
        as_of_date = date(2024, 3, 15)
        sources = ["ces", "laus", "claims"]

        state = vintage_harness.reconstruct_state(as_of_date=as_of_date, sources=sources)
    ...
    def test_reconstruct_uses_latest_available_vintage(self, vintage_harness, sample_vintages):
        ...
        state_exact = vintage_harness.reconstruct_state(
            as_of_date=as_of_date_exact, sources=[source]
        )

        # Should use exactly 2024-03-01
        assert state_exact.vintage_dates[source] == date(2024, 3, 1)

    def test_reconstruct_before_first_vintage(self, vintage_harness, sample_vintages):
        ...
        with pytest.raises(VintageReconstructionError) as exc_info:
            vintage_harness.reconstruct_state(as_of_date=as_of_date, sources=[source])

        assert "No vintage available" in str(exc_info.value)
        assert source in str(exc_info.value)

    def test_reconstruct_with_missing_source(self, vintage_harness, sample_vintages):
        """Test reconstruction when a requested source doesn't exist."""
        as_of_date = date(2024, 2, 15)
        sources = ["ces", "nonexistent_source"]

        with pytest.raises(VintageReconstructionError) as exc_info:
            vintage_harness.reconstruct_state(as_of_date=as_of_date, sources=sources)
    ...
    def test_reconstruct_with_empty_sources_list(self, vintage_harness):
        ...
        with pytest.raises(ValueError) as exc_info:
            vintage_harness.reconstruct_state(as_of_date=as_of_date, sources=[])

        assert "sources list cannot be empty" in str(exc_info.value).lower()

    def test_reconstructed_state_properties(self, vintage_harness, sample_vintages):
...
class TestVintageHonestyValidation:
    ...
    def test_validate_no_future_data_leakage(self, vintage_harness, sample_vintages):
    ...
    def test_detect_future_data_in_reconstruction(
        ...
        future_dates = pd.date_range(
            start="2020-01-01", end="2024-03-01", freq="MS"  # Past as_of_date!
        )
    ...
    def test_validate_vintage_dates_before_as_of_date(self, vintage_harness, sample_vintages):
    ...
    def test_validate_data_timestamps_before_vintage_date(self, vintage_harness, sample_vintages):
...
class TestEdgeCases:
    ...
    def test_short_series_handling(self, vintage_manager, vintage_harness):
    ...
    def test_partial_source_availability(self, vintage_manager, vintage_harness):
        ...
        with pytest.raises(VintageReconstructionError) as exc_info:
            vintage_harness.reconstruct_state(
                as_of_date=as_of_date, sources=["ces", "laus", "claims"]
            )

        assert "claims" in str(exc_info.value)

    def test_reconstruct_with_allow_partial_sources(self, vintage_manager, vintage_harness):
        ...
        state = vintage_harness.reconstruct_state(
            as_of_date=as_of_date, sources=["ces", "laus", "claims"], allow_partial=True
        )
    ...
    def test_empty_vintage(self, vintage_manager, vintage_harness):
        ...
        with pytest.raises(Exception):  # VintageError
            vintage_manager.create_vintage(source, pd.DataFrame(), date(2024, 1, 1))
...
class TestVintageHarnessIntegration:
    ...
    def test_full_backtest_workflow(self, vintage_harness, sample_vintages):
        ...
        for backtest_date in backtest_dates:
            # Reconstruct state
            state = vintage_harness.reconstruct_state(as_of_date=backtest_date, sources=sources)
    ...
    def test_reconstruct_with_metadata(self, vintage_harness, sample_vintages):
...
class TestPerformance:
    ...
    def test_reconstruction_performance(self, vintage_harness, sample_vintages):
    ...
    def test_validation_performance(self, vintage_harness, sample_vintages):
