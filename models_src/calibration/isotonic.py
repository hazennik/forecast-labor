...
class IsotonicCalibrator:
    ...
    def __init__(self, out_of_bounds: str = "clip"):
        ...
        logger.debug("isotonic_calibrator_initialized", extra={"out_of_bounds": out_of_bounds})

    def fit(self, y_true: np.ndarray, y_pred_probs: np.ndarray) -> "IsotonicCalibrator":
        """
        ...
        ece_improvement_pct = (
            (ece_improvement / self.ece_before_ * 100) if self.ece_before_ > 0 else 0
        )
    ...
    def transform(self, y_pred_probs: np.ndarray) -> np.ndarray:
        ...
        logger.debug("isotonic_calibrator_transform", extra={"n_samples": len(y_pred_probs)})
    ...
    def fit_transform(self, y_true: np.ndarray, y_pred_probs: np.ndarray) -> np.ndarray:
        """
    ...
    def get_reliability_diagram_data(
        ...
        uncal_data = self._compute_reliability_curve(y_true, y_pred_probs_uncalibrated, n_bins)
        ...
        if y_pred_probs_calibrated is not None:
            ...
            cal_data = self._compute_reliability_curve(y_true, y_pred_probs_calibrated, n_bins)
    ...
    def _compute_reliability_curve(
        self, y_true: np.ndarray, y_pred_probs: np.ndarray, n_bins: int
    ) -> Dict[str, np.ndarray]:
        """
    ...
    def save(self, path: Path) -> None:
        ...
        calibrator_data = {"calibrator": self.calibrator_, "params": self.get_params()}
