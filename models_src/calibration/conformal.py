...
class ConformalPredictor:
    ...
    def __init__(
        ...
        self.confidence_levels = (
            confidence_levels if confidence_levels is not None else self.DEFAULT_CONFIDENCE_LEVELS
        )
    ...
    def _validate_parameters(self) -> None:
        ...
        for conf in self.confidence_levels:
            if not 0 < conf < 1:
                raise ValueError(f"All confidence levels must be in (0, 1), got {conf}")

        if self.adaptive_gamma <= 0:
            raise ValueError(f"adaptive_gamma must be positive, got {self.adaptive_gamma}")
    ...
    def predict_interval(
        self, y_pred: np.ndarray, confidence_level: float = 0.9
        ...
        logger.debug(
            ...
            extra={"n_samples": len(y_pred), "confidence_level": confidence_level},
        )
    ...
    def _compute_adaptive_width(self, y_pred: np.ndarray, base_quantile: float) -> np.ndarray:
        """
        ...
        for i, pred in enumerate(y_pred):
            ...
            kernel_weights = np.exp(
                -distances / (self.adaptive_gamma * np.std(self.y_pred_calibration_))
            )
    ...
    def _compute_coverage_diagnostics(self, y_true: np.ndarray, y_pred: np.ndarray) -> None:
        """
        Compute coverage diagnostics on calibration set.
    ...
    def validate_coverage(
        self, y_true: np.ndarray, y_pred: np.ndarray, confidence_level: float = 0.9
    ) -> Dict[str, float]:
        """
        ...
        logger.info("coverage_validation", extra=metrics)
    ...
    def save(self, path: Path) -> None:
        ...
        logger.info(
            ...
            extra={"path": str(path), "file_size_bytes": path.stat().st_size},
        )
    ...
    def load(cls, path: Path) -> "ConformalPredictor":
        ...
        logger.info(
            ...
            extra={"path": str(path), "n_calibration": predictor.n_calibration_},
        )
