...
class RevisionForecaster(BaseForecaster):
    ...
    def __init__(self, alpha: float = 1.0, fit_intercept: bool = True, random_state: int = 42):
        """
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series, vintage_date: str) -> "RevisionForecaster":
        """
        ...
        self.model_ = Ridge(
            alpha=self.alpha, fit_intercept=self.fit_intercept, random_state=self.random_state
        )
    ...
    def predict(self, X: pd.DataFrame) -> np.ndarray:
        ...
        if X.shape[1] != self.n_features_:
            raise ValueError(
                f"Feature mismatch: Expected {self.n_features_} features, " f"got {X.shape[1]}"
            )
    ...
    def get_feature_importance(self) -> pd.DataFrame:
        ...
        if self.model_ is None:
            raise ValueError(
                "Model must be fitted before computing feature importance. " "Call fit() first."
            )
        ...
        importance_df = importance_df.sort_values("abs_coefficient", ascending=False).reset_index(
            drop=True
        )
    ...
    def save(self, path: Path) -> None:
        ...
        try:
            ...
            logger.info("Saved revision model", path=str(path), model_id=str(self.model_id))
        except Exception as e:
            logger.error(
                "Failed to save revision model", path=str(path), error=str(e), exc_info=True
            )
    ...
    def load(cls, path: Path) -> "RevisionForecaster":
        ...
        try:
            ...
            logger.error(
                "Failed to load revision model", path=str(path), error=str(e), exc_info=True
            )
