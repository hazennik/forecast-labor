...
def extract_signed_bundle(
    ...
    with zipfile.ZipFile(bundle_path, "r") as zf:
        ...
        if verify:
            ...
            if unexpected_files:
                raise TamperDetectedError(f"Bundle contains unexpected files: {unexpected_files}")
    ...
    if verify:
        ...
        if not extracted_artifact_path.exists():
            raise FileNotFoundError(f"Extracted artifact not found: {extracted_artifact_path}")
    ...
    if (extract_dir / "artifact").exists():
        result["artifacts"] = [
            str(f.relative_to(extract_dir)) for f in (extract_dir / "artifact").iterdir()
        ]
