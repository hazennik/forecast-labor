...
class DynamicFactorModel(BaseForecaster):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series, vintage_date: str) -> "DynamicFactorModel":
        ...
        self._train_prediction_model(y_scaled)
        self.is_fitted = True
    ...
    def _preprocess_target(self, y: pd.Series, fit: bool) -> np.ndarray:
        ...
        target = target.ffill().bfill().fillna(float(target.mean()) if not target.dropna().empty else 0.0)
    ...
    def _fit_dynamic_factor(self, X_scaled: np.ndarray) -> None:
        ...
        if n_features < 2:
            logger.warning("dfm_using_pca_fallback", reason="statsmodels_requires_multiple_features")
    ...
    def _fit_pca_fallback(self, X_scaled: np.ndarray) -> None:
        ...
        self.loadings_ = self._pad_loadings(raw_loadings, n_features)
        self.transition_ = self._pad_transition(raw_transition)
        self.log_likelihood_ = [-0.5 * float(np.sum((X_scaled - self.factors_ @ self.loadings_.T) ** 2))]
        self.n_iter_ = 1
    ...
    def _extract_loadings(self, results: Any, n_features: int, n_factors: int) -> np.ndarray:
        ...
        if not np.any(np.abs(loadings) > 0):
            _, _, vt = np.linalg.svd(np.asarray(results.model.endog, dtype=float), full_matrices=False)
    ...
    def _train_prediction_model(self, y_scaled: np.ndarray) -> None:
        """Regress the standardized target on extracted factors."""
        design = np.column_stack([self.factors_, np.ones(len(self.factors_))])
        coef = np.linalg.lstsq(design, y_scaled, rcond=None)[0]
        self.prediction_coef_ = coef[:-1]
        self.prediction_intercept_ = float(coef[-1])
