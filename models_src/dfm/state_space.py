...
def build_transition_matrix(
    ...
    logger.debug(
        ...
        extra={"n_factors": n_factors, "ar_coef": ar_coef, "is_diagonal": True},
    )
...
def build_observation_matrix(
    ...
    if n_factors > n_obs:
        raise ValueError(f"n_factors ({n_factors}) cannot exceed n_obs ({n_obs})")
    ...
    if method == "random":
        ...
        if data.shape[1] != n_obs:
            raise ValueError(f"Data has {data.shape[1]} columns, expected {n_obs}")
...
def validate_state_space_dimensions(
    ...
    # Check loadings
    if loadings.ndim != 2:
        raise ValueError(f"Loadings must be 2D array, got shape {loadings.shape}")
