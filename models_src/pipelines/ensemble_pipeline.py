...
def validate_predictions_dict(predictions: Dict[str, np.ndarray], expected_keys: List[str]) -> None:
    """
...
def optimize_weights(
    ...
    if not result.success:
        logger.warning("Weight optimization did not converge", message=result.message)

    # Convert to dict
    optimized_weights = {name: float(weight) for name, weight in zip(model_names, result.x)}

    logger.info("Optimized ensemble weights", weights=optimized_weights, final_mse=result.fun)
...
class EnsembleForecaster(BaseForecaster):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series, vintage_date: str) -> "EnsembleForecaster":
        """
        ...
        if self.config.optimize_weights:
            logger.info("Optimizing ensemble weights", n_samples=len(X), n_models=len(self.models))
            ...
            logger.info("Ensemble weights optimized", weights=self.optimized_weights)
        else:
    ...
    def predict(self, X: pd.DataFrame) -> np.ndarray:
        ...
        """
        logger.debug("Generating ensemble predictions", n_samples=len(X), method=self.config.method)
        ...
        logger.debug("Ensemble predictions generated", n_predictions=len(result))
    ...
    @classmethod
    def load(cls, path: Path, models: Dict[str, BaseForecaster]) -> "EnsembleForecaster":
        """
        ...
        # Recreate ensemble
        ensemble = cls(models=models, config=ensemble_data["config"])
        ...
        logger.info("Ensemble loaded", path=str(path), n_models=len(models))
