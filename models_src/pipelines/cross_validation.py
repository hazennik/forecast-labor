...
def generate_expanding_window_folds(
    ...
    if data.empty:
        raise ValueError(f"No data available on or before vintage date {config.vintage_date}")
    ...
    for fold_idx in range(config.n_folds):
        ...
        if test_end_idx > len(data):
            logger.warning(f"Insufficient data for fold {fold_idx}, stopping at {fold_idx} folds")
            break
        ...
        if X_train.empty or X_test.empty:
            raise ValueError(
                f"Empty split in fold {fold_idx}: " f"train={len(X_train)}, test={len(X_test)}"
            )

        # Validate chronological order (data leakage prevention)
        if X_train.index.max() >= X_test.index.min():
            raise ValueError(
                f"CRITICAL: Fold {fold_idx} has data leakage - " f"training overlaps with test"
            )
...
def aggregate_cv_metrics(
    ...
    for split in ["train", "test"]:
        ...
        for metric_name in metric_names:
            ...
            values = [fold_result[metrics_key][metric_name] for fold_result in cv_results]
