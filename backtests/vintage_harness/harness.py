...
class VintageHarness:
    ...
    def __init__(self, vintage_base_path: Path):
        ...
        logger.info("VintageHarness initialized", vintage_path=str(self.vintage_base_path))

    def reconstruct_state(
        self, as_of_date: date, sources: List[str], allow_partial: bool = False
    ) -> ReconstructedState:
        """
        ...
        for source in sources:
            try:
                ...
                vintage_data = self.vintage_manager.get_vintage_as_of(
                    source_name=source, as_of_date=as_of_date
                )

                if vintage_data is None:
                    error_msg = (
                        f"No vintage available for source '{source}' " f"on or before {as_of_date}"
                    )
    ...
    def validate_vintage_honesty(self, state: ReconstructedState) -> Tuple[bool, List[str]]:
        """
        ...
        for source, data in state.data.items():
            ...
            if "date" in data.columns:
                try:
                    ...
                    logger.warning(f"Could not validate dates for source '{source}': {e}")
        ...
        for source, data in state.data.items():
            if "date" in data.columns:
                try:
                    ...
                    logger.warning(f"Could not check future data for source '{source}': {e}")

        is_valid = len(errors) == 0

        if is_valid:
            logger.info("Vintage honesty validation passed", sources=len(state.sources_available))
        else:
            logger.error("Vintage honesty validation failed", errors=errors)

        return is_valid, errors

    def get_available_backtest_dates(
        self, sources: List[str], start_date: Optional[date] = None, end_date: Optional[date] = None
    ) -> List[date]:
        """
        ...
        logger.info("Found backtest dates", sources=sources, dates_available=len(common_dates))
