...
class VintageManager:
    ...
    def load_vintage(self, source_name: str, vintage_date: date) -> pd.DataFrame:
        """
        ...
        if not vintage_file.exists():
            raise VintageError(f"Vintage not found: {source_name} on {vintage_date}")
    ...
    def get_vintage_as_of(self, source_name: str, as_of_date: date) -> Optional[pd.DataFrame]:
        """
        ...
        if not available_vintages:
            logger.warning(f"No vintages available for {source_name} on or before {as_of_date}")
            return None

        # Get the latest available vintage
        latest_available = available_vintages[-1]

        logger.info(f"Using vintage from {latest_available} for as_of_date {as_of_date}")
