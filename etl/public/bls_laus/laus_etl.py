...
class LAUSETL(BaseETL):
    ...
    def _fetch_series_batch(
        self, series_ids: List[str], series_dict: Dict[str, str], start_year: int, end_year: int
    ) -> List[Dict]:
        """
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["is_preliminary"] = df["footnotes"].apply(
            lambda x: any("preliminary" in str(f).lower() for f in x)
    ...
    def get_state_employment_summary(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        summary = latest[
            ["state_name", "state_fips", "date", "value", "mom_change", "yoy_change"]
        ].copy()
