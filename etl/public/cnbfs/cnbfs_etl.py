...
class CNBFSETL(BaseETL):
    ...
    def extract(self) -> pd.DataFrame:
        ...
        try:
            ...
            df = df.rename(columns={"cell_value": "value", "time_slot_id": "period"})
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_pivot = df.pivot_table(
            index="date", columns=type_col, values="value", aggfunc="sum"
        ).reset_index()
        ...
        df_pivot.rename(
            columns={k: v for k, v in column_mapping.items() if k in df_pivot.columns}, inplace=True
        )
        ...
        if (
            "business_formations" in df_pivot.columns
            and "high_propensity_applications" in df_pivot.columns
        ):
