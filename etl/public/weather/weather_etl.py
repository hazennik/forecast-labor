                ...
                try:
                    ...
                    if "BEGIN_YEARMONTH" in df_year.columns and "BEGIN_DAY" in df_year.columns:
                        df_year["date"] = pd.to_datetime(
                            df_year["BEGIN_YEARMONTH"].astype(str)
                            + df_year["BEGIN_DAY"].astype(str).str.zfill(2),
                            format="%Y%m%d",
                            errors="coerce",
                        )
                    elif "BEGIN_DATE_TIME" in df_year.columns:
                        df_year["date"] = pd.to_datetime(
                            df_year["BEGIN_DATE_TIME"], errors="coerce"
                        )
                    ...
                    df_year["deaths"] = pd.to_numeric(
                        df_year.get("deaths_direct", 0), errors="coerce"
                    ).fillna(0) + pd.to_numeric(
                        df_year.get("deaths_indirect", 0), errors="coerce"
                    ).fillna(
                        0
                    )
                    df_year["injuries"] = pd.to_numeric(
                        df_year.get("injuries_direct", 0), errors="coerce"
                    ).fillna(0) + pd.to_numeric(
                        df_year.get("injuries_indirect", 0), errors="coerce"
                    ).fillna(
                        0
                    )
                    ...
                    if "damage_property_str" in df_year.columns:
                        df_year["damage_millions"] = df_year["damage_property_str"].apply(
                            self._parse_damage
                        )
                    else:
                    ...
                    if "date" in df_year.columns:
                        df_year = df_year[
                            (df_year["date"] >= pd.Timestamp(start_date))
