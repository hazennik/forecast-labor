...
def create_validated_etl_config(
    ...
    # Define schema rules
    schema_rules = [
        SchemaRule(column_name=col, required=True)
...
def create_monthly_series_config(
    ...
    required_columns = ["date", "series_id"] + series_columns
...
def get_ces_config() -> ETLConfig:
    ...
    return create_monthly_series_config(
        source_name="bls_ces",
        series_columns=["value", "preliminary"],
        enable_reports=True
    )
