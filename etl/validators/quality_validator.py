    ...
    def _setup_rules(self):
        ...
        if self.critical_columns:
            self.add_rule(
                ValidationRule(
                    ...
                    description="Critical columns must not have nulls",
        ...
        if self.unique_keys:
            self.add_rule(
                ValidationRule(
                    name="unique_keys",
                    description="Key columns must form unique identifier",
                    severity=ValidationSeverity.ERROR,
                    check_function=lambda df: check_unique_key(df, self.unique_keys, "unique_keys"),
                )
            )
    ...
    def _check_numeric_ranges(self, df: pd.DataFrame) -> ValidationResult:
        ...
        if range_violations:
            total_violations = sum(
                v["below_min_count"] + v["above_max_count"] for v in range_violations.values()
            )
