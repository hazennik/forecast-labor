...
def test_holiday_regressors():
    ...
    try:
        ...
        for col in regressors.columns:
            ...
            print(
                f"      Non-zero values: {non_zero_count} / {len(regressors)} ({non_zero_pct:.1f}%)"
            )
        ...
        april_2020 = regressors.loc[
            regressors.index.to_period("M") == pd.Period("2020-04", freq="M"), "easter_timing"
        ]
        ...
        nov_2023 = regressors.loc[
            regressors.index.to_period("M") == pd.Period("2023-11", freq="M"), "thanksgiving_timing"
        ]
        ...
        sept_2023 = regressors.loc[
            regressors.index.to_period("M") == pd.Period("2023-09", freq="M"), "labor_day_timing"
        ]
        ...
        if variables_line:
            ...
            for regressor_name in regressors.columns:
                if regressor_name in variables_line:
                    print(
                        f"   ❌ FAIL: '{regressor_name}' found in variables=() (should only be in user=())"
                    )
                    has_duplicates = True

            if not has_duplicates:
                print("   ✅ PASS: No user regressors in variables=() declaration")
