...
def create_historical_vintages(
    ...
    for i in range(num_vintages):
        ...
        history_months = (data_cutoff.year - earliest_data.year) * 12 + (data_cutoff.month - earliest_data.month)
...
def main():
    ...
    if "source" in source_data.columns:
        ...
        if "synthetic" in str(source_type).lower():
            ...
            logger.warning("  docker compose exec etl python -c \"from etl.public.bls_ces.ces_etl import CESETL; import os; CESETL(api_key=os.getenv('BLS_API_KEY')).run()\"")
            return False
