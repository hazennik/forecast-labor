...
def ensure_directories():
    ...
    if not gitignore.exists():
        gitignore.write_text("# Signing keys must never be committed\n*.key\nREVOKED_*.txt\nROTATION_LOG.json\n")
...
def activate_key(version: str) -> bool:
    ...
    if not verify_key(version):
        print(f"❌ Key verification failed, cannot activate")
        return False
    ...
    print(f"   1. Test signing with new key")
    print(f"   2. Keep old key for verification during transition")
    print(f"   3. After all models re-signed, archive old key")
...
def show_status():
    ...
    if ROTATION_LOG.exists():
        ...
        print()
        for event in recent:
            print(f"  {event.get('version', 'unknown'):8} | "
...
See docs/SECURITY_KEY_MANAGEMENT.md for full procedures.
    ...
    parser.add_argument(
        '--version',
        help='Key version (e.g., v3)'
    )
    
    parser.add_argument(
        '--reason',
        default='rotation',
        help='Reason for archival (default: rotation)'
    )
    
    parser.add_argument(
        '--force',
        action='store_true',
        help='Force overwrite if key exists'
    )
