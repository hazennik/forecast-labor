...
def fix_strike_vintage_data(vintage_date: str = "2025-11-29"):
    ...
    logger.info(
        f"Original workers_involved: min={df['workers_involved'].min()}, max={df['workers_involved'].max()}, mean={df['workers_involved'].mean():.2f}"
    )
    ...
    logger.info(
        f"WSU010 stats: min={df[wsu010_col].min()}, max={df[wsu010_col].max()}, mean={df[wsu010_col].mean():.2f}, non-zero={( df[wsu010_col] > 0).sum()}"
    )
    ...
    logger.info(
        f"Fixed workers_involved: min={df['workers_involved'].min():.0f}, max={df['workers_involved'].max():.0f}, mean={df['workers_involved'].mean():.0f}"
    )
    logger.info(
        f"Non-zero workers_involved: {(df['workers_involved'] > 0).sum()} / {len(df)} ({(df['workers_involved'] > 0).sum() / len(df) * 100:.1f}%)"
    )
    ...
    logger.info(
        f"Significant months: {df['is_significant_month'].sum()} / {len(df)} ({df['is_significant_month'].sum() / len(df) * 100:.1f}%)"
    )
    ...
    significant = (
        df[df["is_significant_month"]].sort_values("workers_involved", ascending=False).head(10)
    )
    print(
        significant[
            [
