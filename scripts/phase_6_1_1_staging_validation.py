...
class Phase611Validator:
        ...
        try:
            ...
            for key, description in required_keys.items():
                ...
                if not is_configured:
                    ...
                    logger.warning("api_key_missing", key=key, description=description)
                else:
                    # Don't log actual key values
                    logger.info("api_key_configured", key=key, description=description)
    ...
    def check_infrastructure(self) -> ValidationStep:
        ...
        try:
            ...
            if (
                result.returncode == 0
                or "3/4 services healthy" in output_combined
                or "4/4 services healthy" in output_combined
            ):
                step.status = ValidationStatus.PASSED
                step.details = {
                ...
                step.details = {"stdout": result.stdout, "stderr": result.stderr}
        ...
        try:
            ...
            result = subprocess.run(
                [
                    ...
                    "etl",
                    "python",
                    "/app/scripts/seed_public_data.py",
                ],
                capture_output=True,
                text=True,
            ...
            if result.returncode == 0:
                step.status = ValidationStatus.PASSED
                step.details = {"etl_completed": True, "stdout_preview": result.stdout[:500]}
                logger.info("real_etl_passed")
            else:
                step.status = ValidationStatus.FAILED
                step.error_message = f"ETL execution failed: {result.stderr[:500]}"
                step.details = {"stdout": result.stdout[:500], "stderr": result.stderr[:500]}
                logger.error("real_etl_failed", stderr=result.stderr[:500])

                self.report.issues_discovered.append(
                    "Real ETL failed - check API keys and network connectivity"
                )
        ...
        try:
            ...
            result = subprocess.run(
                [
                    ...
                    "etl",
                    "python",
                    "/app/etl/validators/run_validation.py",
                    "--source",
                    "all",
                    "--mode",
                    "production",
            ...
            if result.returncode == 0:
                step.status = ValidationStatus.PASSED
                step.details = {"validation_passed": True, "stdout_preview": result.stdout[:500]}
        ...
        try:
            ...
            result = subprocess.run(
                [
                    ...
                    "etl",
                    "python",
                    "/app/scripts/run_seasonal_adjustment.py",
                ],
                capture_output=True,
                text=True,
            ...
            if result.returncode == 0:
                ...
                self.report.issues_discovered.append("Seasonal adjustment failed on real data")
    ...
    def train_sample_model(self) -> ValidationStep:
        ...
        logger.info(
            "sample_model_training_deferred",
            reason="Phase 5 training pipeline not yet orchestrated",
        )
    ...
    def generate_final_report(self) -> Tuple[Path, Path]:
        ...
        if any(step.status == ValidationStatus.FAILED for step in self.report.steps):
            ...
            step.status in [ValidationStatus.PASSED, ValidationStatus.SKIPPED]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Phase 6.1.1: Staging Validation with Real Data")
    parser.add_argument(
        "--check-only", action="store_true", help="Only check environment setup (no ETL execution)"
    )
    parser.add_argument(
        "--run-full-validation", action="store_true", help="Run complete validation workflow"
    )
    parser.add_argument(
        "--generate-report",
        action="store_true",
