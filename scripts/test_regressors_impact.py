...
def load_series_from_vintage(series_id: str, vintage_date: str = "2025-11-29"):
    ...
    if "series_id" in df.columns:
        ...
        logger.error("No series_id column in data")
        return None
    ...
    series = pd.Series(
        series_df["value"].values, index=pd.DatetimeIndex(series_df["date"]), name=series_id
    )

    return series


def run_seasonal_adjustment_test(
    series_id: str, series_info: dict, with_all_regressors: bool = True
):
    """
    ...
    try:
        ...
        logger.info(
            f"  Random residuals: {'✅ Yes (p > 0.05)' if p_value and p_value > 0.05 else '❌ No (p ≤ 0.05)'}"
        )
...
def main():
    ...
    for series_id, series_info in MONITORED_SERIES.items():
        result = run_seasonal_adjustment_test(series_id, series_info, with_all_regressors=True)
        if result:
    ...
    if golden_path.exists():
        ...
        for result in results:
            ...
            print(
                f"{sid:<30} {old_p:.6f if old_p else 'N/A':<15} {new_p:.6f if new_p else 'N/A':<15} {change_str:<15} {status:<10}"
            )
