...
def test_degradation_detection():
    ...
    for i in range(5):
        ...
        monitor.record_diagnostics(
            ...
            diagnostics={"m7": m7_value, "m8": 0.50, "q_statistic": 0.45},
            timestamp=datetime(2024, 1, 1) + timedelta(days=i),
        )
    ...
    if result["alert"]:
        ...
        if "trend" in result["alert"]:
            ...
            for stat, trend in result["alert"]["trend"].items():
                print(
                    f"  {stat}: {trend['first']:.3f} → {trend['latest']:.3f} (change: {trend['change_pct']:.1f}%)"
                )
...
def test_summary_report():
    ...
    for i in range(3):
        monitor.record_diagnostics(
            ...
            diagnostics={"m7": 0.45 + (i * 0.05), "m8": 0.50, "q_statistic": 0.42},
            timestamp=datetime(2024, 1, 1) + timedelta(days=i),
        )
